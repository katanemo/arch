...
class PredictHandler():
    def __init__(self, model, tokenizer, device, task = "toxic", hardware_config = "intel_cpu"): 
        self.model = model
...
class GuardHandler():
    def __init__(self, toxic_model, jailbreak_model, hardware_config = "intel_cpu"): 
        ...
        if toxic_model is not None:
            self.toxic_handler = PredictHandler(toxic_model['model'], toxic_model['tokenizer'], toxic_model['device'], "toxic", hardware_config)
        else
            self.single = True
        if jailbreak_model is not None:
            self.jailbreak_handler = PredictHandler(jailbreak_model['model'], jailbreak_model['tokenizer'], jailbreak_model['device'], "jailbreak", hardware_config)
        else
            self.single = True
        self.hardware_config = hardware_config
    ...
    def guard_predict(self, input_text):
        ...
        if not self.single:
            with ThreadPoolExecutor() as executor:
                ...
                toxic_thread = executor.submit(self.toxic_handler.predict, model_1, input_text)
                jailbreak_thread = executor.submit(self.jailbreak_handler.predict, model_1, input_text)
