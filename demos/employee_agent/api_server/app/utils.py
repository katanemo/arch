...
def generate_project_data(conn=None):
...
def generate_salary_history(conn=None):
    if not os.path.exists('salary_history.csv'):
        employees = pd.read_sql_query("SELECT eid FROM employees", conn)
        salary_history = []
        ...
        df_salary_history.to_csv('salary_history.csv', index=False)
    else:
        logger.info(f"{'* ' * 50}\n\nLoading data from CSV File\n\n{'* ' * 50}")
        df_salary_history = pd.read_csv('salary_history.csv')
...
def generate_certifications(conn=None):
    if not os.path.exists('certifications.csv'):
        employees = pd.read_sql_query("SELECT eid FROM employees", conn)
        ...
        df_certifications.to_csv('certifications.csv', index=False)
    else:
        logger.info(f"{'* ' * 50}\n\nLoading data from CSV File\n\n{'* ' * 50}")
        df_certifications = pd.read_csv('certifications.csv')
